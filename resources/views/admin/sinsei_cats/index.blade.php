  ...
  <div class="container-fluid">
    <div class="row">
      <div class="col-md-12">
        ...
        <!--フォームの送信先を指定-->
        <form action="{{ action('Admin\Sinsei_catController@edit') }}" method="post" enctype="multipart/form-data">
            <!--エラーの数だけ$eに入れて$eを表示-->
            @if (count($errors) > 0)
                <ul>
                ...
                <div class="form-group col-md-1">
                    <label for="tail">しっぽの長さ</label>
                        <select name="tail" class="custom-select">
                            @foreach($posts as $p => $tail)
                                <option value="{{ $p }}">$tail</option>
                            @endforeach
                        </select>
            ...
            <div class="form row">
                ...
                {{ csrf_field() }}
                <input type="submit" class="btn btn-primary" value="申請された台帳を登録">
        </form>
    </div>
